...
#shifting
for (i in c(1:step)){
  res[,(ncol(res)+1)] <- shift(res[,2+i], n=i)
  colnames(res)[ncol(res)] <- paste0("pred-stepahead-",i)
}
...
NRMSEs_lstm <- NULL
res <- res[-(1:step),]
for(j in (13:ncol(res))){
  MAPEs_lstm[j-12] <- mape(res$original, res[,j])
  R2s_lstm[j-12] <- R2(res$original, res[,j], form="traditional")
  NRMSEs_lstm[j-12] <- nrmse(res$original, as.numeric(res[,j]))
}
...
ggplot(data=res, aes(x=time))+
  ...
  labs(x="Time", y="WIP[kg]", 
       title ="10 step-ahead prediction with multiple output LSTM",
       subtitle = "real and predicted values",
       color="Legend") +
  geom_line(aes(y=`pred-stepahead-10`, color="LSTM"), size=1)+
  theme(plot.title = element_text(hjust=0.5))+ # plot in to the middle
