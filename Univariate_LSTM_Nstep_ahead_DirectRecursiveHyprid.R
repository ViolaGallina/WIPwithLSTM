...
window = 15   # the number of historical data used for prediction is defined
...
X_train <- array(x_train, dim=c(dim(x_train)[1], window,1))
#X_train <- as.matrix(x_train)
Y_train <- array(y_train, dim= c(dim(x_train)[1], steps, 1))
...
Y_test <- array(y_test, dim=c(dim(y_test)[1], steps,1))
...
for(i in 1:steps){
  ...
  # use the prediction in the next training phase
  x_train_new <- model %>% predict(X_train)
  x_train <- x_train[,-1] # delete first column containing the oldest data
  x_train <- cbind(x_train, x_train_new) # add the predictions in the last column 
...
#setattr(results, "row.names", 1:nrow(results))
...
ggplot(data=results, aes(x=timeScale))+
  ...
  labs(x="Time", y="WIP[kg]", 
       title ="5 step-ahead prediction with direct recursive multistep LSTM",
